...
DateSlider = Class.create({
	initialize : function(p_sBarId, p_sStartDate, p_sEndDate, p_iStartYear, p_iEndYear, options) {
		...
		} else if ( p_iStartYear instanceof Date) {
			this.barStartDate = p_iStartYear;
			this.iStartYear = p_iStartYear.getFullYear();
			console.log(p_iStartYear.getFullYear());
		}

		if ( typeof p_iEndYear == "number") {
			...
			this.barEndDate = p_iEndYear;
			this.iEndYear = p_iEndYear.getFullYear();
		}
		...
		if (this.options.dragHandles == false)
			this.numberOfDays = this.oStartDate.getDiffDays(this.oEndDate);
		this.centerDate = Date.today();
		if (this.options.centerDate != null)
			this.centerDate = Date.parse(options['centerDate']);
	...
	createShiftPanel : function(p_sBarId, p_sStartDate, p_sEndDate) {
		...
		new Draggable(l_oShiftPanel, {
			...
			onStart : function() {
				if (sliderReference.options.onStart)
					sliderReference.options.onStart();
			}
		});
	},
